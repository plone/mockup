...
## [5.7.0-alpha.3](https://github.com/plone/mockup/compare/5.7.0-alpha.2...5.7.0-alpha.3) (2026-09-17)

### Bug Fixes

- **pat-structure:** Prevent datatables alerts in specific column layouts. ([bbfff28](https://github.com/plone/mockup/commit/bbfff284cbbca0f9e1bea3e84e2c66f1f31daeda))

    Co-authored-by: Codex <noreply@openai.com>

- **pat-structure:** Scan rows when attached. ([c11fb38](https://github.com/plone/mockup/commit/c11fb384d013700698b7490223ae274cd4384727))
...
- **core:** restrict the formatjs locale-data context to base languages ([3b8878b](https://github.com/plone/mockup/commit/3b8878b442269854154d3434678c44cce28249b8))

    core/intl-loader.js dynamically imports `locale-data/${baseLang}.js` from
    the ~766-file @formatjs/intl-datetimeformat/locale-data directory. Webpack
    turns that into a context and emits one lazy chunk per file (~110 MB of
    chunks plus ~220 MB of source maps), and the production build runs out
    of the default Node heap. baseLang never carries a region or script
    suffix, so the ~450 variant files were never requestable.
    See plone/mockup#1640.

    Constrain the context with webpackInclude to base-language files
    (de.js, pt.js, ...). Regional variants still resolve through the polyfill's
    own fallback (pt-BR -> pt, de-AT -> de), as they did before. The include is
    matched against the absolute file path, so it must not be anchored at "./".

    Move the polyfill-force import and the locale-data context into a separate
    lazy intl-polyfill.js, loaded only after feature detection fails, so the
    context map is not part of the pat-structure chunk.

    Measured on a production build, 3587 -> 3145 chunk files, 767 -> 324
    locale chunks, dist 380 MB -> 170 MB, bundle.min.js -13 KB (webpack's
    runtime chunk-hash map shrinks with the chunk count).

    closes [plone/mockup#1640](https://github.com/plone/mockup/issues/1640)

### Maintenance

- **bundle:** update @patternslib/dev to 4.2.0 and Babel to 8 ([acdfe34](https://github.com/plone/mockup/commit/acdfe34e4e11d36ca357ee5dfabd7b9398c34930))

    @patternslib/dev 4.2.0 ships the production source map setup this
    package needed (nosources-source-map, css/sass source maps off) and
    upgrades to Babel 8 targeting ES6+ browsers.

    Babel 8 removes the allowDeclareFields and allExtensions options of
    @babel/preset-typescript (both are now the default behaviour), so drop
    them from the babel config, the svelte runes-in-module webpack rule and
    the jest transformer. @babel/core is declared as a devDependency, as the
    babel config and the jest transformer require it directly, and pinned
    to 8 via a pnpm override: babel-jest otherwise resolves its @babel/core
    peer to a second 7.x instance, under which the Babel 8 presets do not
    strip TypeScript and the .ts test suites fail to parse.
...
- **pat-tinymce:** construct the modal implementation for the link modal ([e72149a](https://github.com/plone/mockup/commit/e72149a9348aa6d62726e2083eee39b0385e4f78))
    ...
    constructs the _registered_ pattern — now the thin registration module,
