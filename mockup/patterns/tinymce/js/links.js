define([
  ...
  var AnchorLink = LinkType.extend({
    init: function() {
      ...
      this.anchorNames = [];  // Sorted list of anchor names
      this.anchorData = {};   // Data by anchor names
      this.populate();
    },

    load: function(element) {
      var name = this.tiny.dom.getAttrib(element, 'data-val');
      this.$select.select2('val', name);
    },
    ...
    populate: function() {
      ...
      this.anchorNames = [];  // Sorted list of anchor names
      this.anchorData = {};   // Data by anchor names
      var nodes, node, i, j, name, title;

      if (self.tiny.inline) {
        nodes = $('.mce-content-body').find('a.mceItemAnchor,img.mceItemAnchor,a.mce-item-anchor,img.mce-item-anchor');
      } else {
        nodes = self.tiny.dom.select('a.mceItemAnchor,img.mceItemAnchor,a.mce-item-anchor,img.mce-item-anchor');
      }
      for (i = 0; i < nodes.length; i = i + 1) {
        ...
        if (name !== '') {
          self.anchorNames.push(name);
          self.anchorData[name] = {node: node, name: name, title: name};
        }
      }

      if (self.tiny.inline) {
        nodes = $('.mce-content-body').find(self.linkModal.options.anchorSelector);
      } else {
      ...
      if (nodes.length > 0) {
        for (i = 0; i < nodes.length; i = i + 1) {
          ...
          if ($(node).parents('.mosaic-IDublinCore-title-tile').length) {
            name = 'content';
          }
          /* okay, ugly, but we need to first check that this anchor isn't already available */
          var found = false;
          for (j = 0; j < self.anchorNames.length; j = j + 1) {
            var data = self.anchorData[self.anchorNames[j]];
            if (data.name === name) {
              found = true;
          ...
          if (!found) {
            self.anchorNames.push(name);
            self.anchorData[name] = {node: node, name: name, title: title, newAnchor: true};
          }
        }
      }
      if (self.anchorNames.sort) {
        self.anchorNames = self.anchorNames.sort(function (a, b) {
          return $(self.anchorData[a].node).offset().top - $(self.anchorData[b].node).offset().top;
        });
      }
      if (self.anchorNames.length > 0) {
        for (i = 0; i < self.anchorNames.length; i = i + 1) {
          var data = self.anchorData[self.anchorNames[i]];
          self.$select.append('<option value="' + data.name + '">' + data.title + '</option>');
        }
      } else {
    ...
    getIndex: function(name) {
      for (var i = 0; i < this.anchorNames.length; i = i + 1) {
        var data = this.anchorData[this.anchorNames[i]];
        if (data.name === name) {
          return i;
        }
      }
