 ...
 *    highlight(string): CSS class that will be used to highlight items on keyboard navigation. ('pattern-search-highlight')
 *    minimumInputLength(integer): Number of characters user must type before search begins. (3)
 *    input(string): Selector for element that will be used for the search input. ('.pattern-search-input')
 *    toggleTarget(string): Selector for element that will be shown when results are found. ('.pattern-search-container')
 ...
 *    resultsTarget(string): Selector for element where results from the server will be inserted. ('.pattern-search-results')
 ...
 *    resultSelector(string): Will be used to select result items from within the result container. ('.pattern-search-result')
 *    resultTemplate(string): Will be used to select result items from within the result container. ('<li class="pat-search-result pat-search-type-<%= Type %>"><a class="pat-search-result-title" href="<%= getURL %>"><%= Title %></a><p class="pat-search-result-desc"><%= Description %></p></li>')
 *    resultTemplateSelector(string): Selector for element to be used in place of resultTemplate. (null)
 *    helpTemplate(string): Template for the element that will contain help messages, such as 'No results found.' or 'Type 3 more characters to search.' ('<div class="pattern-search-help"><%= help %></div>')
 ...
 *
 * Example: example-1
 *    <input type="text" class="pat-search"
 *           data-pat-search="" />
 *
 * Example: example-2
 *    <input type="text" class="pat-search"
...
define([
  ...
  var Search = Base.extend({
    ...
    defaults: {
      ...
      delay: 200,
      blurDelay: 200,
      highlight: 'pattern-search-highlight', // class to add to items when selected
      minimumInputLength: 3, // number of chars user should type before searching
      toggleTarget: '.pattern-search-container', // the element to show/hide when performing search
      toggleClass: 'show',
      positionToggleWithInput: true,
      resultsTarget: '.pattern-search-results', // the element to fill with results
      input: '.pattern-search-input', // input selector
      ...
      resultSelector: '.pattern-search-result',
      resultTemplate: '' +
        '<li class="pattern-search-result pattern-search-type-<%= Type %>">' +
          '<a class="pattern-search-result-title" href="<%= getURL %>">' +
            '<%= Title %>' +
          '</a>' +
          '<p class="pattern-search-result-desc"><%= Description %></p>' +
        '</li>',
      resultTemplateSelector: null,
      helpTemplate: '<div class="pattern-search-help"><%= help %></div>',
    ...
    init: function() {
      ...
      if (!self.query.valid) {
        $.error('No url provided for search results ' + self.$el);
      }
      ...
      self.$input.on('focus.search.patterns', function(e) {
        window.clearInterval(self.blurTimeout);
        self.show();
      });

      self.$input.on('blur.search.patterns', function(e) {
      ...
      if (self.options.toggleTarget) {
        ...
        if (self.$toggle.length) {
          $('html').on('click.search.patterns', function() {
            self.hide();
          });
          self.$toggle.on('click.search.patterns', function(event) {
            event.stopPropagation();
            window.clearInterval(self.blurTimeout);
          });
          self.$input.on('click.search.patterns', function(event) {
            event.stopPropagation();
          });
        }
      ...
      self.on('showing.search.patterns', function(event) {
        self.render(event);
      });
      self.on('searched.search.patterns', function(event) {
        self.render(event);
      });
      self.on('render.search.patterns', function(event) {
        self.render(event);
      });
      self.on('searching.search.patterns', function(event) {
        self.render(event);
      });
      self.on('cachedresult.search.patterns', function(event) {
        self.render(event);
      });
    },
    ...
    _keyEscape: function() {
      this.$input.trigger('blur.search.patterns');
    },
