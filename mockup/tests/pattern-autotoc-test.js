  ...
  ========================== */
    ...
    describe("AutoTOC", function () {
        ...
        it("sets href and id", function () {
            Registry.scan(this.$el);
            expect($("> nav > a:first", this.$el).attr("id")).to.equal(
                "autotoc-item-autotoc-0"
            );
            expect($("> nav > a:first", this.$el).attr("href")).to.equal(
                "#autotoc-item-autotoc-0"
            );
        });
        ...
        it("can be appended anywhere", function () {
            this.$el.attr(
                "data-pat-autotoc",
                "levels: h1;appendTo:.placeholder"
            );
            expect($("> nav", this.$el).length).to.equal(0);
            ...
            ).to.equal("autotoc-nav");
        });
        it("can be prepended anywhere", function () {
            this.$el.attr(
                "data-pat-autotoc",
                "levels: h1;prependTo:.placeholder"
            );
            expect($("> nav", this.$el).length).to.equal(0);
            expect($("div.placeholder > nav", this.$el).length).to.equal(0);
            Registry.scan(this.$el);
