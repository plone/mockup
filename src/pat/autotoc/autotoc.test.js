...
describe("1 - AutoTOC", function () {
    ...
    it("by default creates TOC from h1/h2/h3", async function () {
        ...
        expect(document.querySelectorAll(".nav > li > button").length).toEqual(9);
        expect(
            document.querySelectorAll(".nav > li > button.autotoc-level-1").length
        ).toEqual(4);
        expect(
            document.querySelectorAll(".nav > li > button.autotoc-level-2").length
        ).toEqual(4);
        expect(
            document.querySelectorAll(".nav > li > button.autotoc-level-3").length
        ).toEqual(1);
        expect(
            document.querySelectorAll(".nav > li > button.autotoc-level-4").length
        ).toEqual(0);
    });
    it("sets href and id", async () => {
        ...
        expect(document.querySelectorAll(".nav > li > button")[0].getAttribute("id")).toEqual(
            "autotoc-item-autotoc-0-tab"
        );
        expect(
            document
                .querySelectorAll(".nav > li > button")[0]
    ...
    it("can have custom levels", async function () {
        ...
        expect($("> .nav > li > button.autotoc-level-1", this.$el).length).toEqual(4);
        expect($("> .nav > li > button.autotoc-level-2", this.$el).length).toEqual(0);
    });
    it("can be appended anywhere", async function () {
        ...
        expect($("div.placeholder", this.$el).children().eq(1).attr("class")).toEqual(
            "autotoc-nav nav nav-tabs"
        );
        expect($("div.placeholder", this.$el).children().eq(2).attr("class")).toEqual(
            "autotoc-content tab-content"
        );
    });
    it("can be prepended anywhere", async function () {
        ...
        expect($("div.placeholder", this.$el).children().eq(2).attr("id")).toEqual(
            "first-elem"
        );
    });
    it("by default the first element is the active one", async function () {
        ...
        expect($("> .nav > li > button.active", this.$el).text()).toEqual("Title 1");
    });
    it("the first element with `classActiveName` will be the active", async function () {
        ...
        expect($("> .nav > li > button.active", this.$el).text()).toEqual("Title 2");
    });
