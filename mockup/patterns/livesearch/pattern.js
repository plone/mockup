 ...
 *    search_handlers(object): Stores information for each searchbox on the page. ({})
 *    LSHighlight(string): Constants for better compression. ('LSHighlight')
 *
 * Documentation:
 *    # Example
 *
 *    {{ example-livesearch }}
 *
 * Example: example-livesearch
 *    <form action="/@@search">
 *      <input type="text" class="pat-livesearch" value="" />
 *      <div class="LSResult"></div>
  ...
  'mockup-patterns-base'
    ...
    var LiveSearch = Base.extend({
        ...
        init: function() {
            ...
            self.$result = self.$form.find('div.LSResult');
            self.$shadow = self.$form.find('div.LSShadow');
            ...
            function _keyhandlerfactory(self) {
                ...
                function _keyUp() {
                    ...
                    var $cur = self.$shadow.find('li.LSHighlight').removeClass(self.options.LSHighlight),
                        $prev = $cur.prev('li');

                    if (!$prev.length) {
                        $prev = self.$shadow.find('li:last');
                    }
                    $prev.addClass(self.options.LSHighlight);
                    return false;
                }
                ...
                function _keyDown() {
                    ...
                    $next.addClass(self.options.LSHighlight);
                    return false;
                }

                function _keyEscape() {
                    // hide results window
                    self.$shadow.find('li.LSHighlight').removeClass(self.options.LSHighlight);
                    self.$result.hide();
                }
                ...
                function _submit() {
                    ...
                    // and open it
                    var $target = self.$shadow.find('li.LSHighlight a').attr('href');
                    if (!$target) {
                        return;
                    }
