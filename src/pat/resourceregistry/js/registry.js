...
var RegistryResourceListItem = BaseView.extend({
    ...
    deleteClicked: function (e) {
        ...
        if (
            window.confirm(
                _t("Are you sure you want to delete the ${name} resource?", {
                    name: this.options.name,
                })
            )
        ) {
...
var RegistryBundleListItem = RegistryResourceListItem.extend({
    ...
    buildClicked: function (e) {
        ...
        if (this.options.registryView.dirty) {
            window.alert(
                _t("You have unsaved changes. Save or discard before building.")
            );
        } else {
...
export default BaseResourcesPane.extend({
    template: _.template(
