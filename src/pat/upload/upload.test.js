define(["expect", "jquery", "pat-registry", "mockup-patterns-upload"], function (
    expect,
    $,
    registry,
    Upload
    ...
    describe("Upload", function () {
        describe("Div", function () {
            ...
            it("default attributes", function () {
                ...
                expect($(".upload-area", this.$el).hasClass("dz-clickable")).to.be.equal(
                    true
                );
                expect($(".dz-message", this.$el).length).to.equal(1);
                expect($(".dz-message", this.$el).hasClass("dz-default")).to.be.equal(
                    false
                );
                expect($(".dz-message p", this.$el).html()).to.equal(
                    "Drop files here..."
                );
            });
            it("change className data option", function () {
                ...
                registry.scan(this.$el);
                expect($(".pat-upload", this.$el).hasClass("drop-zone")).to.be.equal(
                    true
                );
            });
            it("update clickable data option to false", function () {
                ...
                registry.scan(this.$el);
                expect($(".pat-upload", this.$el).hasClass("dz-clickable")).to.be.equal(
                    false
                );
            });
            it("update clickable data option to false", function () {
