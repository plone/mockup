define([
    ...
    describe("Resource registry", function () {
        ...
        it("loads resource data", function () {
            ...
            expect(
                this.$pat.find(".resource-entry .field-url input").attr("value")
            ).to.equal("patterns/autotoc");
        });

        it("loads bundle data", function () {
            this.$pat.find(".bundle-list-item-plone a").trigger("click");
            expect(
                this.$pat.find(".resource-entry .field-resources input").length
            ).to.equal(2);
        });
